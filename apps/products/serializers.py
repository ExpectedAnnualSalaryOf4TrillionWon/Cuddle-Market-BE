...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'condition_status', 'trade_method', 'location', 'status',
            'view_count', 'like_count', 'images', 'created_at', 'updated_at',
        ]
    ...
    def validate_category(self, value):
        ...
        if value not in allowed:
            raise serializers.ValidationError(f"카테고리는 {allowed} 중 하나여야 합니다.")
        return value

    def validate_condition_status(self, value):
        allowed = ['new', 'like_new', 'used', 'damaged']
        if value not in allowed:
            raise serializers.ValidationError(f"상품 상태는 {allowed} 중 하나여야 합니다.")
        return value
    ...
    def validate_trade_method(self, value):
        ...
        for method in value.split(','):
            if method not in allowed:
                raise serializers.ValidationError(f"거래 방식은 {allowed} 중 하나 이상이어야 합니다.")
        return value